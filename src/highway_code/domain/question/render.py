...
class CliRenderQuestion:
    ...
    def render_propositions(self, question: Question) -> None:
        ...
        if len(sub_title_list) > 0:
            self.cli_render.echo(sub_title_list[0])
        self.cli_render.echo(f"{rendered_proposition[0]} {rendered_proposition[1]}")

        if len(propositions) == 2:
            return

        if len(sub_title_list) == 2:
            self.cli_render.echo(sub_title_list[1])
        if len(propositions) == 4:
            self.cli_render.echo(f"{rendered_proposition[2]} {rendered_proposition[3]}")
        else:
            self.cli_render.echo(f"{rendered_proposition[2]}")

    def render_correction(self, question: Question, message: str, is_correct_answer: bool) -> None:
        color = "green" if is_correct_answer else "red"
        size = len(message) + 4
        line = "".ljust(size, "-")
        self.cli_render.echo_color(line, color)
        self.cli_render.echo_color("| " + message + " |", color)
        self.cli_render.echo_color(line + "\n", color)
...
class CliRenderQuestionManager:
    ...
    def ask_answer(self, message: str) -> str:
        message = self.cli_render.message_color(message, "yellow")
        input_answer = self.cli_render.get_input(message)
        return input_answer.replace(" ", "")

    def render_error(self, error: str) -> None:
        self.cli_render.echo_color(error, "red")

    def ask_question_again(self, message: str) -> bool:
        message = self.cli_render.message_color(message, "yellow")
        return self.cli_render.confirm(message)

    def show_congratulation(self, message: str) -> None:
        self.cli_render.echo(message)
