...
def get_mocks(questions: List[Question]) -> MocksStruct:
    question_repository_mock = Mock()
    question_repository_mock.get_all_question.return_value = questions
    seal(question_repository_mock)

    translation_mock = Mock(spec_set=TranslationInterface)
    translation_mock.load_translation()
    translation_mock.translate.return_value = "translated message"
    seal(translation_mock)

    cli_render_question_manager_mock = Mock(spec_set=CliRenderQuestionManager)
    seal(cli_render_question_manager_mock)

    cli_render_question_mock = Mock(spec_set=CliRenderQuestion)
...
def test_selection_question() -> None:
    question = question_factory_test()
    questions = [question]
    mocks = get_mocks(questions=questions)
...
def test_selection_question_multiple() -> None:
...
def test_selection_question_form_id() -> None:
...
def test_selection_question_form_non_existing_id() -> None:
...
def test_selection_question_on_unknown_country() -> None:
...
def test_selection_question_on_empty_list() -> None:
    questions: List[Question] = []
    mocks = get_mocks(questions=questions)
